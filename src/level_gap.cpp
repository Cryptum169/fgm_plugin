...
void laserCallback(const sensor_msgs::LaserScan& msg) {
    ...
    // ROS_INFO_STREAM(angle_boundary[std::min_element(angle_boundary.begin(), angle_boundary.end())]);
    // ROS_INFO_STREAM(angle_boundary[std::max_element(angle_boundary.begin(), angle_boundary.end())]);
    pub.publish(vis_arr);
